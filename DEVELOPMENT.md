...
```bash
git clone https://github.com/bybyKD/septian-kevin-derio-portfolio.git minimal-dev-portfolio
...
The application should now be available at the origin shown by the `next dev` output (see `allowedDevOrigins` in `next.config.ts`).
...
If you're working on a different React project and want to reuse the custom components from this repository, visit [septian-kevin-derio-portfolio.vercel.app/components](https://septian-kevin-derio-portfolio.vercel.app/components) for installation instructions and component documentation.
...
```bash
pnpm capture       # Capture screenshots into .derio/screenshots
