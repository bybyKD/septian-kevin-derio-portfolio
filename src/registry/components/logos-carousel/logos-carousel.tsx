...
export function LogosCarousel({
  ...
  // The consumer may configure the column count in two equivalent ways: the
  // `columnCount` prop or a `--column-count` CSS variable (set via class). The
  // variable is read once from the mounted element so both ways resolve to the
  // same number of grid tracks and rendered columns. Relying on one without
  // the other used to leave empty tracks that never filled the container.
  const [cssColumnCount, setCssColumnCount] = useState<number | null>(null)

  useLayoutEffect(() => {
    const el = containerRef.current
    if (!el) return

    const raw = getComputedStyle(el).getPropertyValue("--column-count").trim()
    if (!raw) return

    const parsed = Number.parseInt(raw, 10)
    if (!Number.isNaN(parsed) && parsed > 0) {
      setCssColumnCount(parsed)
    }
  }, [])
  ...
  const effectiveColumnCount =
    columnCount !== DEFAULT_COLUMN_COUNT
      ? columnCount
      : (cssColumnCount ?? DEFAULT_COLUMN_COUNT)

  const columns = useMemo(
    () => distributeLogos(Children.toArray(children), effectiveColumnCount),
    [children, effectiveColumnCount]
  )
